...
//			LOGGER.info("State " + newState.getName() + " added to the StateMachine.");
	...
	public boolean update(final Eventable event, StateVertix newState, EmbeddedBrowser browser,
		...
		// cloneState不为空时，表示检测到重复
		// 注意SFG添加SV时的重复检测是靠方法.equals()，所以可以改写此方法
		StateVertix cloneState = this.addStateToCurrentState(newState, event);

		// 如果当前状态已经存在，则取出SFG中存在的状态，赋值给newState，并将当前StateMachine的状态指向SFG中的状态，目的是为了防止重复对页面状态进行搜索
		if (cloneState != null) {
			newState = cloneState;
		}
		...
		if (cloneState == null) {
			if (CrawljaxController.checkKSG()) {
				StateVertix.setKeyContent(previousState, currentState);
			}
