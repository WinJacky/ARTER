...
import com.crawljax.core.oraclecomparator.StateComparator;
...
public class CrawljaxController implements CrawlQueueManager {
	...
	// 事件执行顺序关系集合
	public static Map<TagElement, List<TagElement>> preClickConstraintMap;

	// 爬取方式
	public static CrawlStrategy crawlStrategy;

	// 分词、扩词之后的关键词
	public static List<Keyword> keywords = new ArrayList<Keyword>();

	public static SplitWords segment = new SplitWords();
	...
	public void setStartCrawl(long startCrawl) {
	...
	public CrawljaxController(final CrawljaxConfiguration config) {
		...
		crawlStrategy = config.getCrawlSpecification().getCrawlStrategy();

		preClickConstraintMap = config.getCrawlSpecification().getPreClickConstraintMap();
...
//		        "Crawl depth: " + configurationReader.getCrawlSpecificationReader().getDepth());
	...
	public static boolean checkKSG() {
		return CrawljaxController.crawlStrategy == CrawlStrategy.KSG;
	}

	public static void setCrawlStrategy(CrawlStrategy crawlStrategy) {
		CrawljaxController.crawlStrategy = crawlStrategy;
	}
}
