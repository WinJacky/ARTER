...
public final class WebDriverBackedEmbeddedBrowser implements EmbeddedBrowser {
	...
	public synchronized boolean fireEvent(Eventable eventable) {
		try {
			...
			if (webElement != null) {
				// 移动到指定元素
				((JavascriptExecutor) browser).executeScript("arguments[0].scrollIntoView(false);", webElement);
	...
	@Override
	public List<WebElement> getWebElements(Identification identification) {
		try {
			return browser.findElements(identification.getWebDriverBy());
		} catch (WebDriverException e) {
			throw wrapWebDriverExceptionIfConnectionException(e);
		}
	}
